...
class Player {
  constructor(tileSize) {
    ...
    // Whether the player has collected a key for the current level.
    this.hasKey = false;
  ...
  draw() {
    ...
    // If the player has a key, draw a tiny key badge on the avatar.
    if (this.hasKey) {
      fill(240, 200, 30);
      const bx = this.pixelX() + this.ts * 0.18;
      const by = this.pixelY() - this.ts * 0.22;
      rect(bx - 6, by - 4, 12, 8, 3);
      circle(bx - 8, by, 4);
    }
  }
